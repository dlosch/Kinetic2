...
partial class LanguageHelper {
    ...
    private sealed class CSharpLanguageHelper : LanguageHelper {
        ...
        internal override bool TryGetStringSpan(SyntaxToken token, string text, scoped in Loc location, out int skip, out int take) {
            ReadOnlySpan<char> s;
            switch (token.Kind()) {
    ...
    static int SkipEscapedStringCharacters(ref ReadOnlySpan<char> text, int characters) {
            ...
            if (text[i] == '\\' && text.Length > i + 1) {
                switch (text[i + 1]) {
                    ...
                    case 'U' when text.Length > i + 9: // \U00HHHHHH
                        if (text[i + 4] != '0' || text[i + 5] != 0) {   // if not \U0000, then: 2-char UTF32
                            characters--;
                        }
                        i += 9;
                        break;
        ...
        if (i >= text.Length) {   // take everything
            text = default;
            return text.Length;
        }
        else {
...
internal abstract partial class LanguageHelper {
    ...
    internal abstract bool TryGetStringSpan(SyntaxToken token, string text, scoped in Loc location, out int skip, out int take);
...
internal readonly struct Loc {
    ...
    public Loc(int line, int column, int offset, int length) {
        Line = line;
        Column = column;
        Offset = offset;
        Length = length;
    ...
    public static readonly Loc Zero = new(0, 0, 0, 0);
